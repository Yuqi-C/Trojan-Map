...
class TrojanMap {
 public:
  ...
  void TravellingTrojan_Brute_force_aux(int cur_node, double cur_cost, std::vector<std::vector<double>>& weights, std::vector<std::vector<int>>& all_path,
                                      std::vector<int>& cur_path, double& min_cost);

  //TSP Backtracking auxiliary
  void TravellingTrojan_Backtracking_aux(int cur_node, double cur_cost, std::vector<std::vector<double>>& weights, std::vector<std::vector<int>>& all_path,
                                      std::vector<int>& cur_path, double& min_cost);

  //use DFS to detect cycle
  bool DFS(std::string cur_id, std::unordered_map<std::string, int>& visited, std::string parent);

  //Topology auxiliary
  void FindTopology(std::vector<std::string>& result, std::vector<std::string>& locations,
